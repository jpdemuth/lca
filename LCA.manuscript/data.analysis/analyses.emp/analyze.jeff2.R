...
for(i in 1:15){
  png(filename = paste("~/Desktop/Dropbox/papers/LCA/results/jeff/dataset", 
                       i, ".png", sep=""), width = 600, height = 400)
  foo <- AnalyzeCrossesMM(get(paste("data", i, sep="")), Cmatrix = "MP", 
                          model.sum = 0.95, even.sex = F, 
                          graph=T, max.models = 70000)
  dev.off()
  write.csv(foo[[2]], file=paste("dataset", i, ".results.csv", sep=""), row.names=F)
  write.csv(foo[[3]], file=paste("dataset", i, ".waics.csv", sep=""), row.names=F)
